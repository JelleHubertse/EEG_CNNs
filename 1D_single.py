...
class CNN(nn.Module):
    def __init__(self, lrate, binary, batch_size=1, save=True):
        ...
        if binary:
            self.out = nn.Linear(
                in_features=128*1276, out_features=2, bias=True)  # change to 50 if all layers enabled in forward()
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("-me", "--max_epochs", type=int,
                        default=30, help="maximum training epochs")
