    ...
    def get_num_correct(self, preds, labels):
        ...
        if labels.shape != torch.Size([len(labels)]):
            labels = torch.mean(torch.mean(
                torch.mean(labels, dim=1), dim=1), dim=1).round().long()
...
if __name__ == "__main__":
    ...
    EEG_data.load_data()
    data = EEG_data.data
    targets = EEG_data.labels_numerical
    label_mapping = EEG_data.label_mapping

    # Recoding the labels for the specific task
    if args.binary_class:
        # recoding the labels into binary vowel/consonants
        vowels = [0, 1, 7, 8]  # goose, thought, fleece, trap in label_mapping
    ...
    train_tensor = torch.from_numpy(
        ICA.filtered._data).reshape(160, 14, -1).double().to(CNN.device)
    targets_tensor = torch.Tensor(
        targets).double().unsqueeze(dim=1).to(CNN.device)

    # Splitting into the training/test sets
    targets_tensor = targets_tensor.reshape(160, 1, -1).to(CNN.device)
    ...
    full_train_tensor = torch.cat(
        (train_data, train_labels), dim=1).unsqueeze(dim=1).to(CNN.device)
    full_val_tensor = torch.cat(
        (val_data, val_labels), dim=1).unsqueeze(dim=1).to(CNN.device)
    full_test_tensor = torch.cat(
        (test_data, test_labels), dim=1).unsqueeze(dim=1).to(CNN.device)
