...
if __name__ == "__main__":
    ...
    for i in participants_included:
        ...
        # ICA.plot_raw(save=True, show=False)
        # ICA.plot_sources(save=True, show=False)
        # ICA.plot_components(save=True, show=False)
        # ICA.plot_psd(save=True, show=False)
        # ICA.plot_overlay(save=True, show=False)
    ...
    full_train_tensor = torch.cat(
        (train_data, train_labels), dim=1).to(CNN_.device)
    full_val_tensor = torch.cat(
        (val_data, val_labels), dim=1).to(CNN_.device)
    full_test_tensor = torch.cat(
        (test_data, test_labels), dim=1).to(CNN_.device)
